...
function Grammar(text) {
  ...
  this.getSequenceFirsts = function (sequence) {
    ...
    for (var j in sequence) {
      ...
      epsilonInSymbolFirsts |=
        this.firsts[symbol] == undefined || this.firsts[symbol].length == 0;
  ...
  function initializeFirsts(grammar) {
    ...
    do {
      ...
      for (var i in grammar.rules) {
        var rule = grammar.rules[i];
        var nonterminalFirsts = getOrCreateArray(
          grammar.firsts,
          rule.nonterminal
        );
        ...
        if (rule.development.length == 1 && rule.development[0] == EPSILON) {
          ...
          notDone |= collectDevelopmentFirsts(
            grammar,
            rule.development,
            nonterminalFirsts
          );
  ...
  function initializeFollows(grammar) {
    ...
    do {
      ...
      for (var i in grammar.rules) {
        ...
        if (i == 0) {
          var nonterminalFollows = getOrCreateArray(
            grammar.follows,
            rule.nonterminal
          );
        ...
        for (var j in rule.development) {
          ...
          if (isElement(symbol, grammar.nonterminals)) {
            ...
            var afterSymbolFirsts = grammar.getSequenceFirsts(
              rule.development.slice(parseInt(j) + 1)
            );
...
function Rule(grammar, text) {
  ...
  this.equals = function (that) {
    ...
    if (
      parseInt(this.development.length) != parseInt(that.development.length)
    ) {
      return false;
    }
...
function BasicItem(rule, dotIndex) {
  ...
  this.newItemsFromSymbolAfterDot = function () {
    var result = [];
    var nonterminalRules = this.rule.grammar.getRulesForNonterminal(
      this.rule.development[this.dotIndex]
    );
  ...
  this.newItemAfterShift = function () {
    if (
      this.dotIndex < this.rule.development.length &&
      this.rule.development[this.dotIndex] != EPSILON
    ) {
  ...
  this.equals = function (that) {
    return (
      ...
      parseInt(this.dotIndex) == parseInt(that.dotIndex)
    );
  };

  this.toString = function () {
    return (
      this.rule.nonterminal +
      " -> " +
      this.rule.development.slice(0, this.dotIndex).join(" ") +
      "." +
      (isElement(EPSILON, this.rule.development)
        ? ""
        : this.rule.development.slice(this.dotIndex).join(" "))
    );
  };
}
...
function BasicLR1Item(rule, dotIndex) {
  ...
  this.newItemsFromSymbolAfterDot = function () {
    ...
    var firstsAfterSymbolAfterDot = this.rule.grammar.getSequenceFirsts(
      this.rule.development.slice(this.dotIndex + 1)
    );
  ...
  this.equals = function (that) {
    return (
      zuper.equals(that) && includeEachOther(this.lookAheads, that.lookAheads)
    );
  };

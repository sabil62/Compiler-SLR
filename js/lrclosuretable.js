//function that creates the closure table and the canonical items, receiving the grammar as a parameter
function LRClosureTable(grammar) {
  ...
  function updateClosure(kernel) {
    for (var i = 0; i < kernel.closure.length; ++i) {
      var newItemsFromSymbolAfterDot =
        kernel.closure[i].newItemsFromSymbolAfterDot();
    ...
    for (var i in kernel.keys) {
      ...
      if (targetKernelIndex < 0) {
        ...
        for (var j in newKernel.items) {
          lookAheadsPropagated |= newKernel.items[j].addUniqueTo(
            kernels[targetKernelIndex].items
          );
        }
      }
